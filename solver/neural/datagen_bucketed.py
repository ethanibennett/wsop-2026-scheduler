"""24/7 BUCKETED data generation for the Stud 8 value net (the dataset grind).

Generates (bucketed-PBS -> per-bucket CFV) examples fast enough to run
continuously across cores. The trick: a board's bucket share-matrix is the
expensive part and depends only on the board, so we compute it ONCE per board
and then solve many bucketed range samples on it (amortization). Records are
JSONL, consumable by train.py (featurize handles the bucketed schema).

Pure Python. Throughput scales with cores via datagen-24-7.sh (separate
processes, unique shard tags).

CLI:
  python3 datagen_bucketed.py --street 7 --out data/st7 --tag w0 \
      --boards 200 --per-board 30 --iters 150 --samples 60
  python3 datagen_bucketed.py --street 7 --out data/st7 --tag w0 --forever
"""
...
from bucket_resolve import sample_share_matrix, resolve_bucketed
from datagen import up_count, sample_range

DECK = [r + s for r in "23456789TJQKA" for s in "cdhs"]


def sample_board(street: int, rng: random.Random, n_dead: int = 0):
    d = DECK[:]
    rng.shuffle(d)
    uc = up_count(street)
    up = [d[:uc], d[uc:2 * uc]]
...
def _pot(street: int, rng: random.Random) -> float:
    bb = 8 if street >= 5 else 4
    return float(2 * rng.randint(1, 8) * bb)


def generate_bucketed(out_dir: str, street: int, boards: int, per_board: int = 30,
                      iters: int = 150, samples: int = 60, seed: int = 0,
                      tag: str = "w0", shard_size: int = 2000, n_dead: int = 0,
                      n_buckets: int = N_BUCKETS, start_shard: int = 0,
                      progress: Optional[callable] = None):
    """Generate `boards` × `per_board` bucketed examples into out_dir.
...
def _cli():
    import argparse
    p = argparse.ArgumentParser(description="24/7 bucketed Stud 8 datagen.")
    ...
    # restart-safe: continue this tag's shard numbering instead of overwriting
    next_shard = 0
    ...
    while True:
        w, next_shard = generate_bucketed(
        ...
        if not a.forever:
            break
        seed += 1_000_003          # fresh stream each batch
...
def _selftest():
    ...
    out = tempfile.mkdtemp(prefix="stud8_bucketgen_")
    try:
        # tiny board (n_dead shrinks the matrix board) for a fast test
        n, ns = generate_bucketed(out, street=7, boards=1, per_board=3, iters=60,
        ...
        print(f"ok: datagen_bucketed self-tests pass ({n} bucketed examples, "
