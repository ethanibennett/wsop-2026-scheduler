...
fires; `time` is the label), titles, and bodies freely. `PHASES` maps the year to
Ellie's rotations and drives the phase label. When you want different nudges per
phase, fill in `PHASE_NUDGES[phaseId]` and we'll rebuild the scheduler on phase
boundaries.
