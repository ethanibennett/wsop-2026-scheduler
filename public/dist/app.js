  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      for (let i = 0; i < d.length; i += 4) {
        ...
        const val = inverted < 100 ? Math.max(0, inverted * 0.3) : inverted > 160 ? 255 : Math.round((inverted - 100) / 60 * 255);
        d[i] = d[i + 1] = d[i + 2] = val;
      }
...
function CalendarView({ allTournaments, mySchedule, onToggle, gameVariants, venues, onSetCondition, onRemoveCondition, onToggleAnchor, onSetPlannedEntries, buddyEvents, buddyLiveUpdates }) {
  ...
  const sortedEvents = useMemo(() => {
    return [...todayEvents].filter((t) => {
      ...
      const ta = /* @__PURE__ */ new Date(`${a.date} ${a.time && a.time !== "TBD" ? a.time : "12:00 AM"}`);
      const tb = /* @__PURE__ */ new Date(`${b.date} ${b.time && b.time !== "TBD" ? b.time : "12:00 AM"}`);
