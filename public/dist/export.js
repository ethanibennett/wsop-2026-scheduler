...
function drawDeepRunOverlay(ctx, w, h, data) {
  ...
  ctx.fillText(data.tournamentName || "", padX, barY + Math.round(barH * 0.1));
  ...
  const pbY = barY + Math.round(barH * 0.4);
  ...
  if (history.length >= 2) {
    ...
    const chartW = pbW;
    const stacks = history.map((h2) => Number(h2.stack) || 0);
...
__name(drawDeepRunOverlay, "drawDeepRunOverlay");
function drawFinalTableOverlay(ctx, w, h, data) {
  ...
  const headerS = Math.round(h * 0.03);
  ctx.font = "600 " + headerS + "px Univers Condensed, Univers, sans-serif";
  ctx.fillStyle = "#f59e0b";
  ctx.fillText("🏆 FINAL TABLE", padX, barY + Math.round(barH * 0.2));
  ...
  ctx.fillText(buyinStr + (data.tournamentName || ""), padX, barY + Math.round(barH * 0.4));
  const plS = Math.round(h * 0.02);
  ...
  if (data.stack && data.bb) {
    ...
    ctx.fillStyle = "rgba(255,255,255,0.5)";
    ctx.font = Math.round(h * 0.016) + "px Univers Condensed, Univers, sans-serif";
    ctx.fillText("(" + bbCount + "bb)", padX, barY + Math.round(barH * 0.9));
  }
  const wms = Math.round(h * 0.014);
  ctx.font = wms + "px Univers Condensed, Univers, sans-serif";
  ctx.fillStyle = "rgba(255,255,255,0.5)";
...
__name(drawFinalTableOverlay, "drawFinalTableOverlay");
function drawCountdownOverlay(ctx, w, h, data) {
  ...
  ctx.fillText(data.venue || "", padX, barY + Math.round(barH * 0.6));
  const countS = Math.round(h * 0.03);
...
__name(drawCountdownOverlay, "drawCountdownOverlay");
...
__name(drawShareBackground, "drawShareBackground");
...
__name(drawWatermark, "drawWatermark");
...
__name(shareOrDownloadCanvas, "shareOrDownloadCanvas");
const VENUE_CANVAS_COLORS = {
  ...
  "WYNN": "#8a3030",
  "ARIA": "#5a3a9a",
  "GOLDEN NUGGET": "#7a6520",
  "RESORTS WORLD": "#2a7a72",
  "SOUTH POINT": "#5e4430",
  "ORLEANS": "#944828",
  "MGM GRAND": "#2a6a3e",
  "MGM NH": "#8a7020",
  "Turning Stone": "#8a6508",
  "TCH": "#a0522d",
  "PRS": "#909090",
  "VEN": "#858585",
  "CAESARS": "#b8962e",
...
__name(getVenueCanvasColor, "getVenueCanvasColor");
...
__name(loadPDFFonts, "loadPDFFonts");
async function generateSchedulePDF(events, title, opts = {}) {
  ...
  try {
    const fonts = await Promise.race([
      loadPDFFonts(),
      new Promise((_, reject) => setTimeout(() => reject(new Error("Font load timeout")), 5e3))
    ]);
  ...
  const PDF_VENUE_COLORS_LIGHT = {
    ...
    "WYNN": "#7a2020",
    "ARIA": "#4a2a8a",
    "GOLDEN NUGGET": "#6a5510",
    "RESORTS WORLD": "#1a6a62",
    "SOUTH POINT": "#4e3420",
    "ORLEANS": "#843818",
    "MGM GRAND": "#1a5a2e",
    "MGM NH": "#7a6010",
    "Turning Stone": "#7a5508",
    "TCH": "#90421d",
    "VEN": "#6a5510",
    "CAESARS": "#a8861e",
    "HARD ROCK": "#0a8e8e"
  ...
  const fmtRange = /* @__PURE__ */ __name((d) => {
    if (!d) return "";
    const dt = new Date(d);
  ...
  function buildRowsFromEvents(evts) {
    const rows2 = [];
    const newDateRowIndices2 = /* @__PURE__ */ new Set();
    const venueColors2 = [];
    let prevDate = "";
    for (const ev of evts) {
      const d = ev.date || "";
      const isNewDate = d !== prevDate;
      if (isNewDate) newDateRowIndices2.add(rows2.length);
      let dateStr = "";
      if (isNewDate) {
      ...
      venueColors2.push(hexRGB(PDF_VENUE_COLORS[venue.abbr] || "#808080"));
    ...
    return { rows: rows2, newDateRowIndices: newDateRowIndices2, venueColors: venueColors2 };
  }
  __name(buildRowsFromEvents, "buildRowsFromEvents");
  ...
  function drawTableSection(sectionRows, sectionVenueColors, sectionDateIndices, startY) {
    doc.autoTable({
      ...
      didParseCell: /* @__PURE__ */ __name(function(data) {
      ...
      willDrawCell: /* @__PURE__ */ __name(function(data) {
        ...
        if (data.section === "body" && data.column.index === 0 && sectionDateIndices.has(data.row.index) && data.row.index > 0) {
          doc.setDrawColor(...LINE_CLR);
          doc.setLineWidth(0.3);
          doc.line(mg, data.cell.y, pw - mg, data.cell.y);
        }
      }, "willDrawCell"),
      didDrawCell: /* @__PURE__ */ __name(function(data) {
      ...
      didDrawPage: /* @__PURE__ */ __name(function(data) {
        doc.setFont(FONT_BODY, "normal");
        doc.setFontSize(6.5);
        doc.setTextColor(...TEXT_MUT);
  ...
  __name(drawTableSection, "drawTableSection");
  function drawSectionTotal(evts, afterY) {
    ...
    for (let ci = 0; ci < currencies.length; ci++) {
      ...
      const label = currencies.length > 1 ? "TOTAL MAXIMUM BUY-INS (" + (SYMBOL_LABEL[sym] || sym) + ")" : "TOTAL MAXIMUM BUY-INS";
      doc.setFont(FONT_BODY, "bold");
      doc.setFontSize(8);
  ...
  __name(drawSectionTotal, "drawSectionTotal");
  ...
  __name(drawSectionHeader, "drawSectionHeader");
  if (opts.groupByBuyin) {
    const groups = BUYIN_RANGES.map((range) => __spreadProps(__spreadValues({}, range), {
...
__name(generateSchedulePDF, "generateSchedulePDF");
function drawSchedulePage(ctx, w, h, pageEvents, pageNum, totalPages, title, opts) {
  ...
  __name(getCanvasVenueColor, "getCanvasVenueColor");
  ctx.fillStyle = cBG;
  ctx.fillRect(0, 0, w, h);
  const pad = 60;
...
__name(drawSchedulePage, "drawSchedulePage");
...
__name(generateScheduleImages, "generateScheduleImages");
function drawChipStackStory(ctx, w, h, data) {
  ...
  points.forEach((p) => {
    ...
    let label = null;
    let color = "#fbbf24";
    if (entry.is_final_table) {
      label = "FINAL TABLE";
      color = "#f59e0b";
    } else if (entry.is_itm) {
      label = "ITM";
      color = "#22c55e";
    }
    if (entry.is_busted) {
  ...
  const xLabelSize = Math.round(h * 0.01);
  ...
  ctx.font = "600 " + Math.round(h * 0.04) + "px Univers Condensed, Univers, sans-serif";
  ctx.fillStyle = "#22c55e";
  ctx.textAlign = "center";
...
__name(drawChipStackStory, "drawChipStackStory");
function drawSeriesScorecard(ctx, w, h, data) {
  ...
  const drawStat = /* @__PURE__ */ __name((label, value, x, y, color) => {
    ctx.font = Math.round(h * 0.014) + "px Univers Condensed, Univers, sans-serif";
    ctx.fillStyle = "rgba(255,255,255,0.45)";
    ctx.letterSpacing = "1px";
    ctx.fillText(label.toUpperCase(), x, y);
    ctx.letterSpacing = "0px";
    ctx.font = "600 " + Math.round(h * 0.03) + "px Univers Condensed, Univers, sans-serif";
...
__name(drawSeriesScorecard, "drawSeriesScorecard");
...
__name(drawDeepRunStandalone, "drawDeepRunStandalone");
function drawCountdownStory(ctx, w, h, data) {
  ...
  const detailY = Math.round(h * 0.5);
  if (data.buyin) {
...
__name(drawCountdownStory, "drawCountdownStory");
...
function drawFinalTableCard(ctx, w, h, data) {
  ...
  ctx.font = "700 " + Math.round(h * 0.05) + "px Univers Condensed, Univers, sans-serif";
...
__name(drawFinalTableCard, "drawFinalTableCard");
function drawWrapSlide1(ctx, w, h, data) {
  ...
  ctx.fillStyle = "#22c55e";
  ctx.letterSpacing = "4px";
  ctx.fillText("SERIES WRAP", w / 2, Math.round(h * 0.1));
  ctx.letterSpacing = "0px";
  ctx.font = "700 " + Math.round(h * 0.045) + "px Univers Condensed, Univers, sans-serif";
  ctx.fillStyle = "#ffffff";
...
__name(drawWrapSlide1, "drawWrapSlide1");
function drawWrapSlide2(ctx, w, h, data) {
  ...
  ctx.fillStyle = "#22c55e";
  ctx.letterSpacing = "4px";
  ctx.fillText("THE NUMBERS", w / 2, Math.round(h * 0.1));
  ctx.letterSpacing = "0px";
  const row1Y = Math.round(h * 0.2);
  ctx.font = Math.round(h * 0.016) + "px Univers Condensed, Univers, sans-serif";
  ctx.fillStyle = "rgba(255,255,255,0.4)";
  ...
  ctx.font = "600 " + Math.round(h * 0.02) + "px Univers Condensed, Univers, sans-serif";
  ctx.fillStyle = "rgba(255,255,255,0.5)";
...
__name(drawWrapSlide2, "drawWrapSlide2");
function drawWrapSlide3(ctx, w, h, data) {
  ...
  ctx.fillStyle = "#fbbf24";
  ctx.letterSpacing = "4px";
  ctx.fillText("BEST MOMENT", w / 2, Math.round(h * 0.1));
  ctx.letterSpacing = "0px";
...
__name(drawWrapSlide3, "drawWrapSlide3");
function drawWrapSlide4(ctx, w, h, data) {
  ...
  ctx.fillStyle = "#22c55e";
  ctx.letterSpacing = "4px";
  ctx.fillText("GAME MIX", w / 2, Math.round(h * 0.1));
  ctx.letterSpacing = "0px";
  ...
  ctx.fill();
  ctx.font = "700 " + Math.round(h * 0.03) + "px Univers Condensed, Univers, sans-serif";
  ctx.fillStyle = "#ffffff";
...
__name(drawWrapSlide4, "drawWrapSlide4");
function drawWrapSlide5(ctx, w, h, data) {
  ...
  ctx.fillStyle = "#22c55e";
  ctx.letterSpacing = "4px";
  ctx.fillText("FUN FACTS", w / 2, Math.round(h * 0.1));
  ctx.letterSpacing = "0px";
  const padX = Math.round(w * 0.1);
  const padR = w - padX;
  const estHours = (data.eventsPlayed || 0) * 5;
  const hoursY = Math.round(h * 0.2);
...
__name(drawWrapSlide5, "drawWrapSlide5");
function drawMilestoneImage(ctx, w, h, data) {
  ...
  ctx.font = Math.round(h * 0.02) + "px Univers Condensed, Univers, sans-serif";
  ctx.fillStyle = "rgba(255,255,255,0.7)";
...
__name(drawMilestoneImage, "drawMilestoneImage");
...
__name(drawPollEventVsEvent, "drawPollEventVsEvent");
function drawPollOverUnder(ctx, w, h, data) {
  ...
  ctx.fillStyle = "#22c55e";
  ctx.letterSpacing = "3px";
  ctx.fillText("OVER / UNDER", w / 2, Math.round(h * 0.1));
  ctx.letterSpacing = "0px";
  ctx.font = "600 " + Math.round(h * 0.02) + "px Univers Condensed, Univers, sans-serif";
  ctx.fillStyle = "rgba(255,255,255,0.6)";
  ...
  ctx.font = Math.round(h * 0.016) + "px Univers Condensed, Univers, sans-serif";
  ctx.fillStyle = "rgba(255,255,255,0.4)";
  ctx.fillText("CURRENT STACK", w / 2, Math.round(h * 0.3));
  ctx.font = "700 " + Math.round(h * 0.08) + "px Univers Condensed, Univers, sans-serif";
  ctx.fillStyle = "#22c55e";
  ctx.fillText(formatChips(data.currentStack || 0), w / 2, Math.round(h * 0.4));
  if (data.bb) {
    ctx.font = Math.round(h * 0.02) + "px Univers Condensed, Univers, sans-serif";
    ctx.fillStyle = "rgba(255,255,255,0.4)";
...
__name(drawPollOverUnder, "drawPollOverUnder");
...
__name(detectMilestones, "detectMilestones");
function computeScorecardData(trackingData, venueName, tournaments) {
  ...
  const streakText = streak > 1 ? streakType === "cash" ? streak + " cashes in a row" : streak + " event cold streak" : streakType === "cash" ? "1 cash" : "1 miss";
  ...
  return {
    ...
    biggestCash,
    biggestCashEvent,
    biggestCashPlace: biggestEntry == null ? void 0 : biggestEntry.finish_place,
    gameBreakdown,
    eventsPerDay,
    multiEntryCount
  };
}
__name(computeScorecardData, "computeScorecardData");
function loadCardImages(cards) {
  const unique = /* @__PURE__ */ new Map();
  cards.forEach((c) => {
    if (c.suit !== "x") unique.set(c.rank + c.suit, true);
  });
...
__name(loadCardImages, "loadCardImages");
...
__name(drawCanvasCard, "drawCanvasCard");
...
function drawHandOverlay(ctx, w, h, handData, images) {
  ...
  const result = results.length > 0 ? (_a = results[0]) == null ? void 0 : _a.result : null;
  const baseBarPct = 0.28;
  const extraPerOpp = Math.max(0, oppWithCards.length - 1) * 0.06;
  const barH = Math.round(h * Math.min(baseBarPct + extraPerOpp, 0.5));
  const barY = h - barH - Math.round(h * 0.04);
  const padX = Math.round(w * 0.05);
  ctx.fillStyle = "rgba(0,0,0,0.75)";
  ctx.fillRect(0, barY, w, barH);
  ...
  if (hasBoard) {
    ...
    const dividerX = padX + leftW + Math.round((w - padX * 2) * 0.1);
    ...
    if (boardTotalW > rightW) {
      finalBoardCardW = Math.floor((rightW - (boardCards.length - 1) * gap) / boardCards.length);
      finalBoardCardH = Math.round(finalBoardCardW / cardRatio);
    }
    ...
    if (hvTotalW > leftW) {
      finalHVCardW = Math.floor((leftW - (maxHVCards - 1) * gap) / maxHVCards);
      finalHVCardH = Math.round(finalHVCardW / cardRatio);
    }
    ...
    if (totalW > availW) {
      cardW = Math.floor((availW - (maxCards - 1) * gap) / maxCards);
      cardH = Math.round(cardW / cardRatio);
    }
    const contentTop = barY + labelZone;
    const lblS = Math.round(h * 0.014);
    let curY = contentTop;
  ...
  if (results.length > 0) {
    const resS = Math.round(h * (results.length > 1 ? 0.015 : 0.02));
...
__name(drawHandOverlay, "drawHandOverlay");
function drawHandImageOverlay(ctx, w, h, handData, images, tournamentName) {
  ...
  ctx.fillStyle = "rgba(255,255,255,0.7)";
  ctx.fillText(handData.activeGame, w / 2, Math.round(h * 0.1));
  ctx.textAlign = "left";
  ctx.fillStyle = "#22c55e";
  ...
  if (hasBoard) {
    ...
    if (hvCardH > maxCardH) {
      hvCardH = maxCardH;
      hvCardW = Math.round(hvCardH * cardRatio);
    }
    let bCardW = Math.floor((rightW - (boardCards.length - 1) * gap) / boardCards.length);
    let bCardH = Math.round(bCardW / cardRatio);
    if (bCardH > Math.round(h * 0.22)) {
      bCardH = Math.round(h * 0.22);
      bCardW = Math.round(bCardH * cardRatio);
    }
    ctx.font = "600 " + lblS + "px Univers Condensed, Univers, sans-serif";
    ctx.fillStyle = "rgba(255,255,255,0.5)";
    ctx.letterSpacing = "2px";
    ...
    if (cardH > maxCardH) {
      cardH = maxCardH;
      cardW = Math.round(cardH * cardRatio);
    }
    const heroTotalW = heroCards.length * cardW + Math.max(0, heroCards.length - 1) * gap;
    const heroX = Math.round((w - heroTotalW) / 2);
    ctx.font = "600 " + lblS + "px Univers Condensed, Univers, sans-serif";
...
__name(drawHandImageOverlay, "drawHandImageOverlay");
